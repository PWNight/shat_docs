...
import type { SessionPayload, SessionListItem } from "./session.types";
import { env } from "@/env";
...
function getEncodedSessionKey(): Uint8Array {
    ...
    cachedEncodedKey = new TextEncoder().encode(env.SESSION_SECRET);
    return cachedEncodedKey;
}
...
export async function encryptSession(payload: JWTPayload): Promise<string> {
    ...
    const sessionExpiryDays = parseInt(env.SESSION_EXPIRY_DAYS, 10);
    return await new SignJWT(payload)
        .setProtectedHeader({ alg: "HS256" })
        .setIssuedAt()
        .setExpirationTime(`${sessionExpiryDays}d`)
        .sign(encodedKey);
}
